    ...
    with con:
        ...
        if req['session']['new']:
            ...
            if user_name=='':
                ...
                stats=''
                query = "select avg(score) from scores where user='"+user_id+"';"
                mysql_df = pd.read_sql(query, con=con)
                if len(mysql_df) > 0:
                ...
                cur.execute(query)
                query = "create temporary table lid select user,avg(score) as score from scores group by user having count(id)>2 limit 1;"
                cur.execute(query)
